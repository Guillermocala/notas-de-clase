...
int traza(int matriz[100][100], int size);
		...
		if(temp == partition * partition) {
			...
			for (int j = 0; j < partition; j++) {
				for (int k = 0; k < partition; k++) {
					temp_data.datos[j][k] = array_temp[j * partition + k];
				}
			}
	...
	if(getpid() == padre){
		...
		for(int i = 0; i < cant_hijos; i++) {
			if(!hijos[i]) {
				...
				temporal.sumatoria = traza(temporal.datos, temporal.size);
...
void imprimir_datos(struct Data datos) {
  ...
  printf("Sumatoria diagonal principal: %i\n", datos.sumatoria);
}