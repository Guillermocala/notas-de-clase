...
void operacion(struct Data datos);
int calcular_vecindario_hijo1(struct Data datos, int x, int y);
int calcular_vecindario_hijo2(struct Data datos, int x, int y);
				...
				for(int j = 0; j < 10; j++) {
					for (int k = 0; k < info2.filas; k++) {
						for (int l = 0; l < info2.columnas; l++) {
							int temp;
							if(i == 0){
								temp = calcular_vecindario_hijo1(info2, k, l);
								if(temp > 4) {
									info2.datos[i][j] = 0;
								}
							}
							else{
								temp = calcular_vecindario_hijo2(info2, k, l);
								if(temp > 5) {
									info2.datos[i][j] = 0;
								}
							}
