...
class GeneticSolver:
    def __init__(self, populationSize, generationCount, mutationRate, boardSize, elite = 1, fileName = ""):
    ...
    def initialPopulationWithTextFile(self):
        population = []
        with open(self.fileName, "r") as f:
            for line in f.readlines():
                population.append([int(x) for x in line.split(",")])
        ...
        print(f"initial population (txt) is: {solution}")
    ...
    def initialPopulation(self):
        index = 0
        if self.fileName != "":
            self.initialPopulationWithTextFile()
...
path = r'/./Datos/reinas12.txt'
separador = os.path.sep
dir_actual = os.path.dirname(os.path.abspath(__file__))
dir = separador.join(dir_actual.split(separador)[:-1])
newPath = dir + path
# populationSize, generationCount, mutationRate, boardSize, elite, fileName

if 'reinas8' in path:
    boardSolver = GeneticSolver(populationSize=160,generationCount=100, mutationRate=0.01,boardSize=8, elite=1, fileName=newPath)
elif 'reinas12' in path:
    boardSolver = GeneticSolver(populationSize=160,generationCount=100, mutationRate=0.01,boardSize=12, elite=1, fileName=newPath)
else:
